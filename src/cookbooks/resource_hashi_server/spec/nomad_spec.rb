...
describe 'resource_hashi_server::nomad' do
  ...
  context 'adds the consul-template files for nomad' do
    ...
    nomad_metrics_template_content = <<~CONF
      telemetry {
    ...
    it 'creates nomad metrics template file in the consul-template template directory' do
      expect(chef_run).to create_file('/etc/consul-template.d/templates/nomad_metrics.ctmpl')
        .with_content(nomad_metrics_template_content)
    end

    consul_template_nomad_metrics_content = <<~CONF
      ...
      template {
        ...
        # the `contents` option.
        source = "/etc/consul-template.d/templates/nomad_metrics.ctmpl"

        # This is the destination path on disk where the source template will render.
        # If the parent directories do not exist, Consul Template will attempt to
        # create them, unless create_dest_dirs is false.
        destination = "/etc/nomad-conf.d/metrics.hcl"
    ...
    CONF
    it 'creates nomad_metrics.hcl in the consul-template template directory' do
      expect(chef_run).to create_file('/etc/consul-template.d/conf/nomad_metrics.hcl')
        .with_content(consul_template_nomad_metrics_content)
    end
